...
A class can always override a default method to supply a better
behavior.
...
* Superclass wins always against the superinterface - If a
...
* Subtypes win over Supertypes - If a class can inherit a
method from two interfaces, and one is a subtype of
the other,
the
class inherts the method from the subtype
		
* In all other cases the class needs to implement the default
method
...
* <![CDATA[UnaryOperator<T>]]>
- a function from T to T
		
* <![CDATA[BinaryOperator<T>]]>
