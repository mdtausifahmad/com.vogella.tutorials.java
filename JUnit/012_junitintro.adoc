...
To write a JUnit 4 test with the you annotate a method with the `@org.junit.Test` annotation.
...
If that is done correctly, it helps to avoid reading the actual implementation.
...
Build systems like Apache Maven or Gradle in combination with a Continuous Integration Server (like Jenkins) are typically used to execute tests automatically on a regular basis.
