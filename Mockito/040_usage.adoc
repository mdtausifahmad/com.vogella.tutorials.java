...
Via the `ArgumentCaptor class, Mockito can test against arguments of method calls.
This allows to write tests that capture the parameters supplied to a method and test against them.
...
@Mock
private GithubClient.View viewContract;
@Rule public MockitoRule mockitoRule = MockitoJUnit.rule();

@Test
    public void clickOnNote_ShowsDetailUi() {
        // Given a stubbed note
        GitRepository repository = new GitRepository(uri);

        // When open note details is requested
        githubPresenter.openRepositoryDetails(repository);

        // Then note detail UI is shown
        verify(viewContract).showGitRepositoryUi(any(String.class));
    }
----        
