...
A _List_ represents a data structure which allows to dynamically add, access and remove objects of the same type. 
Adding objects to the list is usually done via the `add()` method. 
The `get(int i)` method allows to retrieve the element at position i.
...
The following show contains a small JUnit test for the data structure. I use in the
